...
PROJECT_DIRS = [
    ...
    "working/visual_references/prompts",
    "working/style_approval",
...
def main() -> int:
    ...
    copy_if_missing(templates / "literature-evidence-matrix.csv", project_dir / "working/literature-evidence-matrix.csv")
    copy_if_missing(templates / "delivery-manifest.md", project_dir / "outputs/delivery-manifest.md")
    copy_if_missing(templates / "full-deck-blueprint.md", project_dir / "working/storyboard/full-deck-blueprint.md")
    copy_if_missing(templates / "slide-production-manifest.csv", project_dir / "working/slide-production-manifest.csv")
    copy_if_missing(templates / "slide-spec.yaml", project_dir / "working/slide_specs/slide-001.yaml")
    copy_if_missing(templates / "style-approval-brief.md", project_dir / "working/style_approval/style-approval-brief.md")
    ...
    if not readme.exists():
        readme.write_text(
            ...
            "3. Build and approve four sample slides in `working/style_approval/`.\n"
            "4. Build slide specifications in `working/slide_specs/`.\n"
            "5. Track slide sources in `working/evidence-tracker.csv`.\n"
            "9. Track external literature in `working/literature-evidence-matrix.csv`.\n"
            "10. Save the final PPTX and full-deck montage in `outputs/`.\n",
            encoding="utf-8",
        )
